...
function display_wedding_registry_date_meta_box($wedding_registry){
    $date = get_post_meta($wedding_registry->ID, 'evnt_date', true); 
//    <script>jQuery(function($) {$( "#event-date" ).datepicker({ dateFormat: 'mm/dd/y' });});</script>
    ?>
    <p>Date (mm/dd/yy):</p>
    <input id="event-date" name="event-date" type="text" value="<?php echo $date; ?>" />
    <p>Need new Calendar Jquery Widget activate when here</p>
